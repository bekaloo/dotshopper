...
export const basketSlice = createSlice({
    ...
    extraReducers: (builder => {
        ...
        builder.addCase(removeBasketItemAsync.pending, (state, action)=> {
            ...
            const itemIndex = state.basket?.items.findIndex(i => i.productId === action.meta.arg.productId);
            if (itemIndex === -1 || itemIndex === undefined) return;
            if(action.meta.arg.quantity === state.basket?.items[itemIndex].quantity){
                state.remove = 'Delete' + action.meta.arg.productId
            }else{
                state.status = 'pendingRemoval'+ action.meta.arg.productId;
            }
        });
